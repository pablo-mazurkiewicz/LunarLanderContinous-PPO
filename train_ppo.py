"""
PPO Training Script with Logging and Normalization
FIXED: Saves logs to C:/rl_logs to avoid 'ł' character issue in User path.
"""
import sys
import json
import os
...
SAFE_LOG_DIR = "C:/rl_logs/ppo" 

class LogCallback(BaseCallback):
    """Logs episode returns during training"""
    def __init__(self, log_dir="ppo_logs", seed=0):
        super().__init__()
    ...
    def _on_training_end(self) -> None:
        log_file = os.path.join(self.log_dir, f"returns_seed{self.seed}.json")
...
def train_ppo(seed=0):
    ...
    callback = LogCallback(log_dir="ppo_logs", seed=seed)
    ...
    try:
        ...
        model.save(f"ppo_lunarlander_seed{seed}")
        env.save(f"vec_normalize_seed{seed}.pkl")
        print(f"Zapisano model i statystyki.")
        env.close()
